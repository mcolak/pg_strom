/*
 * Copied from src/backend/access/heap/tuptoaster.c of PostgreSQL
 * to call toast_save_datum() and toast_delete_datum() in extension
 */
 ...
 * toast_extract_datum
 *
 * It extracts a given varlena on the destination address, even if it
 * would be compressed or externally saved. Unlike pg_detoast_datum(),
 * it does not take palloc() to keep an extracted data, so it gives
 * an advantage to reduce number of data copy.
 */
void
toast_extract_datum(void *dest, struct varlena *value, int32 length_be)
{
	...
	{
		Assert(length_be < 0 || VARSIZE_ANY_EXHDR(value) == length_be);
		memcpy(dest, VARDATA_ANY(value), VARSIZE_ANY_EXHDR(value));
	}
}
